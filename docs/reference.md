...
| `add [name] [--same-as P \| --base \| --dir D] [--no-login]` | Add an account. No flag: its own login, logged in right away (browser OAuth) and verified. `--same-as`: another name for P's login. `--base`: adopt `~/.claude`. `--dir`: adopt an existing config dir. Asks for missing parts in a terminal. | |
