...
- **Headless**: no terminal, or `--no-input` (also `CLAUDE_SWITCHER_NO_INPUT=1`).
...
| Command | Does | JSON |
|---|---|---|
| `claude-switcher` | Terminal: pick this project's profile (current preselected), or create one, or more actions. No terminal: same as `status`. | via `status` |
...
| `use [profile] [dir] [--local]` | Map `dir` (default: the current project root) to a profile. Asks for the profile when omitted and a terminal is present. `--local` writes `dir/.claude-switcher` instead. `claude-switcher <profile> [dir]` is a shorthand. | |
| `forget [dir] [--local]` | Remove the mapping (or the `.claude-switcher` file) of `dir`, which may no longer exist. Nothing to forget (it only inherits, or has no mapping) is a no-op that says what applies instead. | |
...
| `add [name] [--same-as P \| --base \| --dir D] [--no-login \| --sso] [--email E]` | Add an account. No flag: its own login, logged in right away (browser OAuth) and verified. `--sso` and `--email` go to `claude auth login` and imply its own login. `--same-as`: another name for P's login. `--base`: adopt `~/.claude`. `--dir`: adopt an existing config dir. Asks for missing parts in a terminal. | |
| `login [profile] [claude auth login args]` | Log an existing account in again (a login expired, or you want another account behind the name): hands the terminal to `claude auth login`, then verifies with `claude auth status`. A name that is not an account yet offers to add it in a terminal; without one it fails with the `add` command to run. Warns when two accounts end up on one login, and when there is no terminal to paste a code into. For an alias, logs in its owner. `claude auth login` or `/login` run in a mapped folder log in that folder's account too. | |
| `logout [profile]` | `claude auth logout` for that profile (and every alias sharing it). | |
| `rename [old] [new]` | Rename (asks for what is missing); mappings, aliases and the default follow. The config dir keeps its path. `.claude-switcher` files are not rewritten. | |
...
`$XDG_CACHE_HOME/claude-switcher/update.json` (default `~/.cache/...`), so no
...
At each folder, a mapping in `config.toml` beats a `.claude-switcher` file. A file
...
tool, after a warning; `claude-switcher doctor` shows what is wrong.

With no profiles at all (just installed), `claude` runs untouched and prints a
pointer to `claude-switcher setup`.

When `claude` runs in a directory where nothing matches: with a terminal, the
picker asks (its **+ Add an account** entry adds and logs in a new account on
the spot) and remembers the answer at the project root (the main worktree, or
...
[profiles.work]
config_dir = "/Users/you/.local/share/claude-switcher/profiles/work"
...
| Variable | Effect |
|---|---|
| `CLAUDE_SWITCHER_PROFILE` | Set by `launch` for the session: the profile name. |
...
| `CLAUDE_SWITCHER_UPDATE_URL`, `CLAUDE_SWITCHER_INSTALLER_URL` | Where the latest release and the installer are fetched from (mirrors, tests). |
...
to `claude-switcher` in the same folder. It is never created over an existing
...
that points to it, whatever its name, and nothing else. Variables: `CLAUDE_SWITCHER_BIN_DIR`,
`CLAUDE_SWITCHER_VERSION`, `CLAUDE_SWITCHER_DOWNLOAD_URL` (mirror of the release
...
Release assets: `claude-switcher-<target>.tar.gz` and `.sha256` for
