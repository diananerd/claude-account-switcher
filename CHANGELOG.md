...
- The installer adds `csw`, a short name for `claude-account` (a symlink next
  to it). It is never created over an existing file or command; `--alias NAME`
  picks another name, `--no-alias` skips it, and `uninstall` removes it.
- `doctor` lists the short command.

### Changed

- The docs start with install, setup and daily use, using `csw`; how it works
